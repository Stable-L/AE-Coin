...
const USDT_CONTRACT = "TXLAQ63Xg1NAzckPwKHvzw7CSEmLMEqcdj"; // USDT TRC20
const SUNSWAP_ROUTER = "TKzxdSv2FZKQrEqkKVgp5DcwEXBEKMg2Ax"; // SunSwap V2
...
async function waitForTronWeb() {
  return new Promise((resolve, reject) => {
    ...
    const timer = setInterval(() => {
      ...
      if (i++ > 20) {
        clearInterval(timer);
        reject("TRON wallet not detected");
      }
    }, 500);
  });
}
...
document.getElementById("connectBtn").onclick = async () => {
  try {
    ...
    alert("Please open with Tron wallet (TronLink / Trust Wallet / OKX)");
  }
};
...
async function loadAECBalance() {
  ...
  document.getElementById("aecBalance").innerText =
    balance / 1_000_000 + " AEC";
}

/* ===============================
   CALCULATE PRICE (FIXED UI)
================================ */
...
document.getElementById("buyBtn").onclick = async () => {
  try {
    ...
    const aecAmount = Number(document.getElementById("buyAmount").value);
    if (aecAmount <= 0) return;

    const usdtAmount = aecAmount * 0.272;
